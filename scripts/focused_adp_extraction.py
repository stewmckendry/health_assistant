#!/usr/bin/env python3
"""
Focused ADP extraction - only extract clinically relevant pages with LLM enhancement
"""
...
class FocusedADPExtractor(EnhancedADPExtractor):
    ...
    def extract_page_range(self, path: str, start_page: int, end_page: int, adp_doc: Optional[str] = None) -> List:
        ...
        for i, match in enumerate(section_matches):
            ...
            section = adp_extractor.ADPSection(
                adp_doc=adp_doc,
                part=part,
...
def main():
    ...
    analysis_file = Path("tests/dr_off_agent/test_outputs/adp_toc_analysis.json")
    
    if not analysis_file.exists():
        print("❌ TOC analysis file not found. Run analyze_adp_toc.py first.")
        return
    
    with open(analysis_file) as f:
        analysis = json.load(f)
    
    print("Focused ADP Extraction with LLM Enhancement (Parallel)")
    print("=" * 60)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Create partial functions with worker IDs
        tasks = []
