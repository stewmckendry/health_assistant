...
class VectorClient:
    ...
    def __init__(
        self,
        persist_directory: str = "data/processed/dr_off/chroma",
        timeout_ms: int = 1000,
        max_workers: int = 3
    ):
        """
    ...
    async def search_adp(
        ...
        where = {}
        if device_category:
            # Filter by device category in metadata
            where = {"$or": [
                {"section": {"$contains": device_category}},
                {"subsection": {"$contains": device_category}}
            ]}
    ...
    async def search_odb(
        ...
        where = {}
        if drug_class:
            where = {"therapeutic_class": {"$contains": drug_class}}
